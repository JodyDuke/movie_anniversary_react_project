      ...
      <div className="all-titles-sidebar">
        ...
        {this.props.data.nodeTitles.map((e, k) => {
          if (e.id !== this.props.data.currentTitle.id) {
            let image =
              tmdb.images.secure_base_url +
              tmdb.images.poster_sizes[0] +
              e.poster_path;
            return (
              <div
                ...
                key={e.id}
                id={e.id}
                onClick={e => this.props.titleChange(e)}
              >
                <img src={image} alt={e.title + "poster"} />
                <h5>
                  {e.birthday}
                  <sup>th</sup> Anniversary
                </h5>
              </div>
            );
          } else return null;
      ...
      <div>
        ...
        {currentData.map((e, k) => {
          ...
          if (e.profile_path !== null) {
            currentProfileImage =
              tmdb.images.secure_base_url +
              tmdb.images.poster_sizes[1] +
              e.profile_path;
          }
          return (
            <div key={k} className="cast-node">
...
class Modal extends Component {
  ...
  changeTitle(e) {
    const newCurrentTitle = this.state.nodeTitles.filter(
      obj => obj.id === parseInt(e.currentTarget.id, 10)
    );

    const url = [
      tmdb.url +
        tmdb.movie +
        newCurrentTitle[0].id +
        tmdb.videos +
        config.TMDB_KEY,
      tmdb.url +
        tmdb.movie +
  ...
  render() {
    ...
    if (this.state.currentTitleCredits !== undefined) {
      const temp = this.state.currentTitleCredits.crew.find(
        e => e.job === "Director"
      );
      director = temp.name;
    }
    ...
    let posterImage =
      tmdb.images.secure_base_url +
      tmdb.images.backdrop_sizes[2] +
      this.state.currentTitle.backdrop_path;
    let currentTitleImage =
      tmdb.images.secure_base_url +
      tmdb.images.poster_sizes[1] +
      this.state.currentTitle.poster_path;
    return (
      <div
      ...
      >
        <div className="modal">
          <div
            className={
              this.state.nodeTitles.length > 1 ? "info" : "info-single-title"
            }
          >
            ...
            {this.state.nodeTitles.length > 1 ? (
              <OtherTitles data={this.state} titleChange={this.changeTitle} />
            ) : null}

            <div className="current-title-info">
              <h1>{this.state.currentTitle.title}</h1>
              <h3>
                {this.state.currentTitle.birthday}
                <sup>th</sup> Anniversary
              </h3>
            ...
            <div className="current-title-image">
              <img
                alt={this.state.currentTitle.title + " poster"}
                ...
                <div className="budget">
                  {this.state.currentTitleMiscInfo.budget > 0 ? (
                    <p>
                      ...
                      <span>
                        ${numberWithCommas(
                          this.state.currentTitleMiscInfo.budget
                        )}
                      </span>
                    </p>
                  ) : null}
                  {this.state.currentTitleMiscInfo.revenue > 0 ? (
                    <p>
                      ...
                      <span>
                        ${numberWithCommas(
                          this.state.currentTitleMiscInfo.revenue
                        )}
                      </span>
                    </p>
                  ) : null}
