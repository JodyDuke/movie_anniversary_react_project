...
class Main extends Component {
  ...
  getMovies(props) {
    ...
    Promise.all(
      ...
      .then(() => {
        const addToArr = calendarMap(
          this.state.month,
          this.state.year,
          addBirthday(this.state)
        );
        let totalCalendarArr = this.state.totalCalendarSession;

        for (let i = 0; i < totalCalendarArr.length; i++) {
          if (
            totalCalendarArr[i].month === this.state.month &&
            totalCalendarArr[i].year === this.state.year
          ) {
        ...
        if (totalCalendarArr.length > 1) {
          if (
            totalCalendarArr[0].month > this.state.month &&
            totalCalendarArr[0].year > this.state.year
          ) {
  ...
  render() {
    return (
      <div className="main">
        {window.location.pathname.includes("movies") ? null : (
          <Header
        ...
        <Route
          path="/movies/:id"
          render={() => <Modal data={this.state.totalCalendarSession} />}
        />
        <Route
          path="/settings"
          render={() => (
            <Settings
              ...
              currentRegion={this.state.countrySelect}
        ...
        <Route
          ...
          render={() => (
            <SearchResults
              ...
              yearsSettings={this.state.yearsSelect}
